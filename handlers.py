...
SELECTING_PACKAGE = range(2, 3)
ENTERING_PROXY_TO_ADD = range(3, 4)
SELECTING_PROXY_TO_REMOVE = range(4, 5)
ENTERING_PRICE_DURATION, ENTERING_PRICE_BDT = range(5, 7)
ENTERING_PAYMENT_METHOD_NAME = range(7, 8)
ENTERING_EXCHANGE_RATE = range(8, 9)
...
async def payment_methods_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = "💳 *Payment Methods*\n\n" + "\n".join([f"• {m['name']}" for m in methods]) if methods else "💳 *Payment Methods*\n\nNo payment methods are currently available."
...
async def pending_orders_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in pending:
        ...
        admin_message = f"🔔 *New Balance Request*\nUser: {user_info.full_name} (`{p['user_id']}`)\nMethod: {p['payment_method']}\nAmount: {p['amount']} {p['currency']}\nTXN: `{p['txn_id']}`"
...
async def admin_add_payment_method_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("➕ Please send the name of the new payment method.")
    return ENTERING_PAYMENT_METHOD_NAME

async def admin_add_payment_method_receive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    method_name = update.message.text
    db.add_payment_method(method_name)
    await update.message.reply_text(f"✅ Payment method '{method_name}' added.")
    await start(update, context)
    return ConversationHandler.END
...
add_payment_method_conv_handler = ConversationHandler(entry_points=[CallbackQueryHandler(admin_add_payment_method_start, pattern='^admin_add_payment$')], states={ENTERING_PAYMENT_METHOD_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_add_payment_method_receive)]}, fallbacks=[CallbackQueryHandler(back_to_main_handler, pattern='^back_to_main$')])
